...
exports.get_posts = function(req, res, next) {
  const query = Post.find().populate('_status').populate('_user').populate('categories').populate('liked_users');
...
exports.post_create_post = function(req, res, next) {
  ...
  if(!req.body || !req.body.title || !req.body.description || !req.body.body) {
    return errorHandler.handleAPIError(400, `Post must have a title, description, body and category`, next);
  }
...
exports.post_update_get = function(req, res, next) {
  async.parallel({
    post: function(callback) {
      const id = req.params.postId;
      Post.findById(id, callback).populate('categories');
    },
    categories: function(callback) {
      Category.find(callback).sort( { created_at: -1} );
    },
...
exports.post_update_put = function(req, res, next) {
  if(!req.body || !req.body.title || !req.body.description || !req.body.body) {
    return errorHandler.handleAPIError(400, `Post must have a title, description, body and _category`, next);
  }
